...
class QIS_GAN_x4(nn.Module):
    ...
    def __init__(self, scale_ratio, n_select_bands,  n_bands, image_size, feat_dim=128,
        ...
        imnet_in_dim_64 = self.feat_dim + self.guide_dim + 2
    ...
    def forward(self, LR_HSI,HR_MSI ):
        ...
        NIR_feature_64 = self.query_64(lr_spe, coord_64, guide64)  # BxCxHxW
...
if __name__ == '__main__':
    ...
    model =QIS_GAN_x4( scale_ratio=4, n_select_bands=4,  n_bands=145, image_size=128, feat_dim=64,
                 guide_dim=64, H=128, W=128, spa_edsr_num=4, spe_edsr_num=4, mlp_dim=[256, 128], NIR_dim=33).cuda()
    hr = torch.rand(1,4,128,128).cuda()
    lr = torch.rand(1,145,32,32).cuda()
    T=model(lr,hr)
    print(T[0].shape)
